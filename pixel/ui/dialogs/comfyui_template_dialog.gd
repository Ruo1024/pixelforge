...
func _ready() -> void:
	title = Strings.text("DIALOG_COMFY_TEMPLATES")
...
func show_manager() -> void:
	_status.text = Strings.text("COMFY_TEMPLATE_HELP")
	popup_centered()
...
func _build_ui() -> void:
	...
	_add_button(
		top,
		Strings.text("COMFY_IMPORT_WORKFLOW"),
		func() -> void: _file_dialog.popup_centered_ratio()
	)
	_id_edit = LineEdit.new()
	_id_edit.placeholder_text = Strings.text("COMFY_TEMPLATE_ID")
	top.add_child(_id_edit)
	_name_edit = LineEdit.new()
	_name_edit.placeholder_text = Strings.text("COMFY_TEMPLATE_NAME")
	...
	_add_button(binding_panel, Strings.text("COMFY_BIND_FIELD"), _bind_selected)
	_add_button(binding_panel, Strings.text("COMFY_SAVE_TEMPLATE"), _save_template)
	_status = Label.new()
...
func _load_workflow(path: String) -> void:
	...
	if not (parsed is Dictionary):
		_status.text = Strings.text("COMFY_WORKFLOW_INVALID")
		return
	_workflow = parsed
	...
	_status.text = Strings.text("COMFY_SLOTS_FOUND") % _slots.size()
...
func _bind_selected() -> void:
	...
	_status.text = Strings.text("COMFY_BOUND") % [field, slot["path"]]
...
func _save_template() -> void:
	...
	_status.text = (
		Strings.text("COMFY_TEMPLATE_SAVED")
		if result.get("ok", false)
		else String(result.get("message", Strings.text("COMFY_WORKFLOW_INVALID")))
	)
