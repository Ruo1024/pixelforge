...
const GROUP_IDS := ["run_status", "input_summary", "preset", "grid", "resample", "quantize", "last_report", "footer"]
...
func _rebuild() -> void:
	...
	var state := Label.new()
	state.name = "RunState"
	state.text = _text("CLEANUP_CARD_STATE_%s" % String(_snapshot.get("run", {}).get("state", "Ready")).to_upper())
	...
	_build_input(body)
	_build_group(body, "PresetGroup", "CLEANUP_CARD_GROUP_PRESET", [_text("CLEANUP_CARD_PRESET_HINT")])
	_build_group(body, "GridGroup", "CLEANUP_CARD_GROUP_GRID", [_text("CLEANUP_CARD_GRID_SUMMARY")])
	_build_group(body, "ResampleGroup", "CLEANUP_CARD_GROUP_RESAMPLE", [_text("CLEANUP_CARD_RESAMPLE_SUMMARY")])
	_build_group(body, "QuantizeGroup", "CLEANUP_CARD_GROUP_QUANTIZE", [_text("CLEANUP_CARD_QUANTIZE_SUMMARY")])
	_build_group(body, "LastReportGroup", "CLEANUP_CARD_GROUP_REPORT", [_text("CLEANUP_CARD_REPORT_EMPTY")])
...
func _build_input(parent: VBoxContainer) -> void:
	...
	var text := "%s · %s · %s" % [String(input.get("kind", "—")), _text("CLEANUP_CARD_INPUT_COUNT") % int(input.get("count", 0)), String(input.get("target", "—"))]
...
func _footer_action() -> String:
	...
	return "cancel_cleanup" if state in ["Queued", "Running"] else ("retry_cleanup_interrupted" if bool(_snapshot.get("run", {}).get("has_interrupted", false)) else "run_cleanup")


func _footer_text() -> String:
	match _footer_action():
		"cancel_cleanup": return _text("CLEANUP_CARD_ACTION_CANCEL")
		"retry_cleanup_interrupted": return _text("CLEANUP_CARD_ACTION_RETRY_INTERRUPTED")
		_: return _text("CLEANUP_CARD_ACTION_START")
