...
func test_mapper_preserves_planner_identity_and_audit_values() -> void:
	var mapper: Script = load(MAPPER_PATH)
	assert_not_null(mapper)
	if mapper == null:
		return
	...
	assert_eq(
		mapped["slot_updates"].map(func(slot: Dictionary) -> String: return slot["slot_id"]),
		["slot-0", "slot-1"],
	)
	assert_eq(
	...
	assert_eq(mapped["actual_cost_usd"], "0.250000")
	assert_eq(mapped["charge_id"], "charge-1")
	assert_eq(mapped["provider_meta"], {"remote_task_id": "remote-1"})
	...
	assert_eq(extra_mapped["slot_updates"].size(), 1)
	assert_eq(extra_mapped["unexpected_slots"].size(), 1)
	assert_true(extra_mapped["unexpected_slots"][0]["unexpected"])
	assert_eq(extra_mapped["received_count"], 2)
	assert_eq(
		extra_mapped["unexpected_slots"][0]["input_snapshot"]["reference_asset_ids"],
		["asset-a"],
	)
...
func test_whole_task_failure_keeps_planned_slots_and_request_audit() -> void:
	...
	assert_eq(mapped["received_count"], 0)
	assert_eq(mapped["actual_cost_usd"], null)
	assert_eq(mapped["provider_meta"], {})
