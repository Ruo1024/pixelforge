...
func test_generate_content_card_routes_run_and_collapsed_state_roundtrips() -> void:
	...
	var run_button: Button = card.get_content_control("PrimaryAction")
	...
	assert_eq(
		card.get_content_control("ProviderAvailability").text,
		Strings.text("GEN_CARD_PROVIDER_AVAILABLE")
	)
	assert_eq(card.get_content_control("Cost").text, Strings.text("GEN_CARD_COST_UNKNOWN"))
	assert_not_null(card.get_content_control("TargetWidth"))
	...
	run_button = card.get_content_control("PrimaryAction")
	assert_false(run_button.disabled)
	assert_eq(run_button.text, Strings.text("CONTENT_ACTION_CANCEL"))
	...
	run_button = card.get_content_control("PrimaryAction")
	assert_false(run_button.disabled)
	assert_eq(run_button.text, Strings.text("CONTENT_ACTION_GENERATE_AGAIN"))
	...
	assert_null(card.get_content_control("PrimaryAction"))
	assert_true(UndoService.undo())
	assert_false(card.collapsed)
	...
	assert_null(collapsed_card.get_content_control("PrimaryActionButton"))
