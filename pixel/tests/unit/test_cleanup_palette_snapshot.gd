...
func test_palette_hash_and_freeze() -> void:
	...
	var first: Dictionary = load(BUILDER_PATH).freeze_palette(
		{"enabled": true, "mode": "fixed_palette", "palette_id": "custom_freeze"}
	)
	...
	assert_eq(
		first["snapshot"],
		first["snapshot"].duplicate(true),
		"the run owns a detached immutable-value snapshot"
	)
	var missing: Dictionary = load(BUILDER_PATH).freeze_palette(
		{"enabled": true, "mode": "fixed_palette", "palette_id": "does-not-exist"}
	)
...
func test_quantize_disabled_freezes_null_palette() -> void:
	if not ResourceLoader.exists(BUILDER_PATH):
		return
	var result: Dictionary = load(BUILDER_PATH).freeze_palette(
		{"enabled": false, "mode": "none", "palette_id": "custom"}
	)
