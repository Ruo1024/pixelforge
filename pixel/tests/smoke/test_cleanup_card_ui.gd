...
func test_fixed_bounds_and_scroll_regions() -> void:
	...
	assert_eq(CardContract.normalize_requested_size("pixel_cleanup", [900, 1200]), Vector2i(800, 1000))
...
func test_all_groups_footer_only_and_runtime_refresh() -> void:
	...
	if view == null:
		return
	assert_eq(view.get_group_ids(), ["run_status", "input_summary", "preset", "grid", "resample", "quantize", "last_report", "footer"])
...
func _view() -> Control:
	...
	view.configure({"params": {"preset_id": "cleanup-16bit-db32", "settings": {}}, "run": {"state": "Ready"}, "input": {"kind": "Output", "count": 2, "target": "32×32"}})
	await wait_process_frames(1)
	return view
