...
func test_two_branch_stage_workspace_runs_saves_and_reopens_with_results() -> void:
	...
	var first_run: Dictionary = MockHarness.run(
	...
	assert_eq(BatchNodeScript.get_visible_asset_ids(first_graph.get_node_params("batch_a")), [])
	assert_eq(
		BatchNodeScript.get_visible_asset_ids(first_graph.get_node_params("output_a")).size(), 4
	)
	ProjectService.set_graph_data(Fixture.GRAPH_ID, first_run["graph"], true)
	var second_run: Dictionary = MockHarness.run(
		GraphScript.from_json(ProjectService.get_graph_data(Fixture.GRAPH_ID)),
		AssetLibrary,
		"generate_b",
		"output_b"
	...
	assert_eq(BatchNodeScript.get_visible_asset_ids(second_graph.get_node_params("batch_b")), [])
	assert_eq(
		BatchNodeScript.get_visible_asset_ids(second_graph.get_node_params("output_b")).size(), 4
	)
	...
	assert_eq(canvas._items_by_id["batch_a_item"].asset_ids.size(), 0)
	assert_eq(canvas._items_by_id["batch_b_item"].asset_ids.size(), 0)
	...
	assert_eq(BatchNodeScript.get_visible_asset_ids(reopened.get_node_params("output_a")).size(), 4)
	assert_eq(BatchNodeScript.get_visible_asset_ids(reopened.get_node_params("output_b")).size(), 4)
