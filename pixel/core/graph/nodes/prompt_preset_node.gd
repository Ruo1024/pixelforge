...
const DEFAULT_PRESET := {
	...
	"prefix": "pixel art, 16-bit style, limited palette, clean pixel grid, retro game asset, DawnBringer palette",
}
...
func validate_params(params: Dictionary) -> Dictionary:
	...
	return {"preset": Dictionary(preset).duplicate(true) if preset is Dictionary else DEFAULT_PRESET.duplicate(true)}
...
func execute(_inputs: Dictionary, params: Dictionary, _ctx: Variant) -> Dictionary:
	...
	return {"prefix": {"prefix": String(preset.get("prefix", "")), "preset_id": String(preset.get("id", ""))}}
