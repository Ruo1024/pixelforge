...
static func _snapshot_is_valid(snapshot: Dictionary) -> bool:
	...
	if String(snapshot["kind"]) == "cleanup":
		...
		if (
			kind not in ["batch", "image_input", "reference_set"]
			or String(snapshot["graph_id"]).is_empty()
			or String(snapshot["source_node_id"]).is_empty()
			or String(snapshot["input_source_node_id"]).is_empty()
			or String(snapshot["source_asset_id"]).is_empty()
		):
			return false
		if kind == "batch":
			if (
				String(snapshot["source_batch_node_id"]).is_empty()
				or String(snapshot["source_slot_id"]).is_empty()
			):
				return false
		elif (
			not String(snapshot["source_batch_node_id"]).is_empty()
			or not String(snapshot["source_slot_id"]).is_empty()
		):
			return false
		if (
			not (snapshot["effective_target_size"] is Array)
			or snapshot["effective_target_size"].size() != 2
		):
			return false
...
static func _cleanup_palette_is_valid(value: Variant) -> bool:
	...
	if (
		...
		or not _has_exact_keys(value, ["palette_id", "content_sha256", "colors_rgba8"])
	):
		return false
	if (
		String(value["palette_id"]).is_empty()
		or String(value["content_sha256"]).length() != 64
		or not (value["colors_rgba8"] is Array)
		or value["colors_rgba8"].is_empty()
	):
		return false
	return true
