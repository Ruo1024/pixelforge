...
2. 测试：卡内可自动化验收转为定向测试并通过；一个整体切片合流时运行全量回归。标注“真实窗口/人工”的项目不得用 headless 结果替代。
...
每个 Beta 集成阶段完成报告必须包含下表，状态只能写：`通过`、`不适用`、`延期登记`、`阻塞`。

状态语义必须分层使用：

以下三项是报告的**结论层级**，不是上表状态列的新枚举；表格单项仍只填写 `通过 / 不适用 / 延期登记 / 阻塞`。

- **工程通过**：代码、自动化与文档达到卡片要求；可以合并，但不代表真人体验或版本可发布。
...
| 自动测试 | 卡内验收标准已转自动化并通过 |  |  |
| 实机/人工 | 整体切片 agent 冒烟已记录；项目所有者人工项在 Beta 候选执行或保持待验 |  |  |
...
| Agent 实机冒烟 | 真实窗口/Computer Use | 一个整体切片的端到端旅程、视觉与异常恢复 | 每个计划定义的整体切片 1 次 |
| 项目所有者人工 | docs/manual-test-*.md | 真机体验、视觉质量、产品 go/no-go | 默认集中到 Beta 候选收口 |
