...
- 新节点写入 OpenAI Image / GPT Image 2、resolution_preset=1080p、
...
- MAX_RESULTS_PER_RUN=16，在 Output、slot、队列和网络前拒绝超限；5..16 张还必须先
  完成用户确认，取消时项目和队列不变；
- seed 恰好为 -1，extra 恰好为空字典；
- 旧 target_width/target_height/ratio_lock/quality、用户 seed、非空 extra 与其他未知
  参数全部拒绝，不读取、不迁移、不保留别名；
- 修改参数即时写 Graph 并进入 Undo，不提供 Apply。

最终提示词按 prefix、自由 prompt、row.text 顺序跳过空值后用逗号空格连接。
Provider 不得追加隐藏业务提示词。开发者预览与真实请求必须调用同一个提示词构建函数。

交付尺寸由 resolution_preset × orientation 固定映射为 12 组。1080p 横向、纵向、
正方形请求分别为 1920×1088、1088×1920、1088×1088，协调器居中裁切为
1920×1080、1080×1920、1080×1080；其他档位请求尺寸等于交付尺寸。逻辑 slot
按有效 row 与行内 count 的全局顺序编号，request 按 descriptor.max_batch 连续拆分，
不跨 row 合并，所有 snapshot 的 requested_seed=-1。
...
input_snapshots 与 request_records 的完整 shape、重试、隐藏结算审计、取消、恢复和
...
slots 和 records 收敛为 interrupted/终态，保留已有成功、隐藏结算审计、provider meta 与
...
ai_generate 与 pixel_cleanup 卡片的尺寸、正文、Header/status/Footer、动作优先级和
滚动唯一遵守批准 `BETA-0.7-FIX-PLAN.md §2.3/§2.6`；错误弹框仍遵守
`BETA-0.7-PLAN.md §12`。默认示例
