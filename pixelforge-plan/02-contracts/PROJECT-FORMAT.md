  ...
  "provenance": {
    ...
    "generation_snapshot": {
      "provider_id":"provider-id", "model_id":"model-id", "mode":"txt2img",
	      ...
	      "actual_width":1080, "actual_height":1080,
...
reference id/SHA 必须等长同序；当前 Graph 主路径的 extra 恰好为空。项目可保存
...
```json
{
  ...
  "prompt_prefix":"", "reference_asset_ids":[], "reference_content_sha256s":[],
	  "target_width":1080, "target_height":1080, "provider_output_size":[1088,1088],
...
kind 只 provider/cleanup；cleanup 的 provider_id=""、meta={}、actual_cost_usd=null、
charge_id=""。
...
attempts 0..3，真正启动后至少 1。charge/meta/actual_cost_usd/PFError 按 PROVIDER-API
精确校验；前三者只作为隐藏结算审计保存，不产生用户费用 UI、估价、预算或累计。
...
Output 固定值：default width=720、min=520、layout max width=1200、top rail=32、
horizontal/vertical padding=16、tile gap=8、max columns=5、max visible rows=3、
tile min=176、target=224、max=320、empty height=520。`n` 是全部 detached=false
槽，不区分状态；Reference 与 Output 共用相同媒体网格与图片 tile。
...
1. queued/running slot→failed/interrupted/retryable=true。对应 record attempts=0 时
   stage=queue；provider attempts>0 为 provider；cleanup attempts>0 为 cleanup。复制安全
	   provider/request/count，保留其他终态、素材、detached、隐藏结算审计与 meta。
