...
│   ├── CURRENT-STATE.md           ← 当前真实状态、人工验收与下一张卡（先读）
...
│   ├── M3-开发规划.md             ← 当前 M3 施工主计划（UX-first + 最小节点闭环）
│   ├── M3-node-graph.md           ← M3 节点功能规格参考
│   ├── M3.1-alpha-readiness.md    ← 当前主平台受邀 alpha 可用性门（M4 前置）
...
1. **读取顺序**：首次接触本项目时，先读 `03-milestones/CURRENT-STATE.md`，再按 `ARCHITECTURE.md → 相关 contract → 当前里程碑文件` 顺序阅读。不需要读完全部文档才动手，但**当前任务卡引用到的契约文件必须读**。
...
6. **每张卡完成后**：运行 `tests/` 下全部测试 + 该卡新增测试；更新对应模块的 `docs/` 注释；在 `CHANGELOG.md` 追加一行；仅在卡片完成、人工结论变化或 go/no-go 决策发生时更新 `CURRENT-STATE.md`。
7. **不确定时的默认值**：遵循 `PRODUCT.md` 的 UX 原则（像素清晰度优先、不打断创作流、批量优先）做判断，并在交付说明中记录所做假设。

## 当前产品执行顺序

下图表达当前产品优先级，不替代各里程碑文件中的技术依赖。M5 技术上可与 M3/M4 并行，但本轮战略上延后，不进入 M3.1 或 M4-V1。

```
M0 ──► M1 ──► M2 ──► M3 ──► M3.1 ──► 受邀 v0.1 alpha ──► M4-V1 ──► [go] M4 ──► M7
        │      │      │                                      │
        └──────┴──────┴──────────────────────────────────────► M5 ──► M6
...
- M1/M2 完成代表本地算法与基础操作可用；M3 代表工作台原型，二者都不自动等于对外 alpha。
- M3.1 通过后才允许称为当前主平台的受邀 v0.1 alpha（纯本地，无 AI 依赖）。
- M4-V1 只验证一个真 Provider 的纵向价值；通过 go/no-go 后才展开完整 M4。
- M4 完成 = 第一个完整生成价值闭环（生成→清洗→切分→入库）。
- M6 完成 = 计划中的 MVP（v0.5）。
- M7 完成 = v1.0（可扩展生态起点）。
...
- **门控现状**：当前使用本地 verify 脚本；GitHub Actions 与三平台导出是后续恢复目标，不得在当前报告中写成已启用能力。
