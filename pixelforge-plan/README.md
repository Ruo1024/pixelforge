...
> 本文档包面向负责开发的 AI 与工程师。2026-07-14，项目所有者已授权把工程证据确认的 Beta 0.6 beta.2 基线集成到本地 `main`，并由唯一执行任务 E 按批准的 Beta 0.7 执行书连续完成 B7-0 至 B7-8。当前只代表本地工程开发；Beta 0.7 尚未人工通过、未构建候选、未 push、未发布。
...
2. `03-milestones/BETA-0.7-PLAN.md`；需要追溯冻结卡片行为时再读 `BETA-0.6-PLAN.md` 与 `BETA-0.6-CARD-DESIGN-SPEC.md`；
...
│   ├── BETA-0.7-PLAN.md          # 当前已批准执行书：生成、Output 与显式清洗收口
...
Beta 0.1–0.5 工程历史（整体人工未签收）
    └─> Beta 0.6 beta.1 已否决
          └─> beta.2 自适应壳工程基线已获授权集成到本地 main
                └─> Beta 0.7 B7-0 已提交、B7-1 已全绿（当前唯一 E 连续执行至 B7-8）
                      └─> B7-8 工程状态报告后停止
```
...
1. **任务卡是边界，不是停顿点**：当前 Goal 连续执行 B7-0 至 B7-8；B7-9、候选构建、push 和发布明确禁止。
2. **契约即法律**：发现契约缺口时提出最小修订；未获批准不得静默造第二套格式或语义。
3. **自动化持续运行**：卡内跑定向测试，整体切片合流时跑全量门禁；自动化红灯不得继续扩张。
4. **自动化开发、最终人工**：Beta 0.7 不使用 Computer Use、不调用真实付费 API；工程自动化不能替代项目所有者人工签收。
5. **状态分层**：工程通过、人工通过、发布通过分别记录，自动化截图不能代替人工；已被项目所有者否决的候选必须撤销旧“工程通过”结论，修复完成后重新建立证据。
6. **复杂 UI 故障先归因**：显示、字体与缩放问题进入隔离诊断分支，按自动化采证—实验—最小修复收口。
7. **保持可回退**：按 B7 卡片本地提交；本轮已获明确权限直接在本地 `main` 开发，但不 push、不发布、不删除旧分支/worktree。
