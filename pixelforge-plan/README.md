...
> 本文档包面向负责开发的 AI 与工程师。当前产品路线已于 2026-07-14 更新：旧 M0–M7 归入 Beta 0.1 工程历史，Beta 0.2 已工程收口，Beta 0.3–0.5 已形成未人工签收的工程候选；Beta 0.6 初始 `0.6.0-beta.1` 已被项目所有者否决，自适应工作区壳修复已形成唯一 `0.6.0-beta.2` 工程候选，等待项目所有者统一人工验收。
                ...
                └─> Beta 0.4 云端生成与结果回流
                      └─> Beta 0.5 工作流复用与 90% 收口
                            └─> Beta 0.6 卡片产品化与画布可读性
                                  └─> beta.1 已否决 → 自适应工作区壳修复工程通过
                                        └─> 唯一 beta.2 候选 → 项目所有者统一人工验收
```
...
1. **任务卡是边界，不是停顿点**：Beta 0.6 的 B6-0 至 B6-8 与后续自适应工作区壳修复已经连续工程收口；当前停止扩张，等待项目所有者统一人工验收。
2. **契约即法律**：发现契约缺口时提出最小修订；未获批准不得静默造第二套格式或语义。
3. **自动化持续运行**：卡内跑定向测试，整体切片合流时跑全量门禁；自动化红灯不得继续扩张。
4. **自动化开发、最终人工**：Beta 0.6 开发期间完全未使用 Computer Use，只运行自动化和脚本截图；现在由项目所有者对唯一 beta.2 候选统一人工验收。
