...
| AI 接入 | Provider 抽象层 | 接口可容纳不同实现；当前 Beta 只发展云端图片模型，本地模型/ComfyUI 无限期延后（见 PROVIDER-API.md） |
...
- ui 层：组件状态、数据映射与几何尽量自动化；当前 Beta 0.6 只使用 UI 场景测试和脚本截图，不使用 Computer Use 做开发期完整测试。
- 卡内运行定向测试，Beta 集成边界运行全量回归；自动化红灯禁止进入下一阶段。项目所有者人工测试集中到 Beta 0.6 全部工程完成后的唯一候选。
