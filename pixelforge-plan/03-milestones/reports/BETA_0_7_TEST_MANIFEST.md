...
- 红灯/实现提交包括 `8c57f70`、`eb5c614`、`5ce0a38`、`5f45fc1`、`1bfe7a2`。
  最终 Output 是 graph-bound batch node 的唯一 UI host，只消费 `result_slots`；固定槽、
  内部滚动、单选工具条、预览、编辑、下载、拆出/拆出全部与 Undo 进入真实画布路径。
- 旧 review/filter/focus/compare、独立 batch card schema、`_add_batch_card` 和 Output 内
  原位抠图/描边菜单已删除。编辑器保存更新选中 slot 的 asset_id，但保留 slot_id、
  原素材和 provenance；不恢复 legacy overwrite 或兼容字段。
- Output、workspace inspector、pixel editor、generation runtime 与 canvas hit/LOD 回归均
  在最终 621/621 全量中通过；B7-4 的协调器/历史 owner 未被 UI 重写。

## 22. B7-6 工程证据

- 红灯/实现提交：`10bac98`、`68347d6`、`5b7fb58`、`e978360`。Cleanup Footer 使用共享
  `GenerationRunCoordinator`、真实 Pipeline 与 TaskQueue；按输入顺序单并发，单项失败
  继续，取消按 worker→operation→wrapper 收敛，retry 只重排原 interrupted snapshot。
- 协调器先登记 cleaned asset 的完整 settings/palette/provenance/report，再写 slot；每次
  完整点击创建新 current Output 并保留历史。Graph 普通 Run 在 Cleanup Ready 停止，
  ai_generate 不能直连 cleanup；旧 Inspector/批次直接清洗和原位替换入口已退役。
- Cleanup operation/coordinator/graph boundary/card/shell 的定向与相关测试均纳入最终全量；
  独立 matte/slice/outline 行为保留，未新增第二个 cleanup writer。

## 23. B7-7 工程证据

- 红灯 `ba6881a`、实现 `5ce8fc7`。默认离线示例只含 prompt_preset、text_prompt、
  reference_set、ai_generate、pixel_cleanup；初始无 Output，cleanup 无输入边，不自动运行。
- 布局按有效卡片边界与 80px 间距计算，并在 generate 与 cleanup 之间预留 600px
  运行时 Output 通道；插入是一条 Undo，不移动/删除既有画布项目。
- 示例使用普通生成、手工连接和显式清洗语义，无示例专用自动连接、自动清洗或外部 PNG。

## 24. B7-8 工程证据

- 红灯/守护提交：`62ba774`、`a4a881f`、`1e627c4`；实现与最终收口提交 `f7b278b`。
  `Strings` 退为 key 兼容入口，所有用户可见文案、Provider/node schema label 和动态 UI
  统一由 en/zh_CN 目录解析；源代码守护拒绝直接常量、裸 English、动态目录绕过和缺 key。
- `test_runtime_language_switch_v2` 覆盖 en→zh_CN→en 的窗口、菜单、Provider、Output、
  Cleanup、示例和错误表面；18 组语言×缩放×窗口几何矩阵共 3691 条断言通过。
- 最终验证：130 scripts、621/621 tests、14818 assertions、1 个既有 orphan、exit 0；
  唯一 `SCRIPT ERROR` 是 `test_plugin_service` 故意加载 `syntax_error` 插件验证隔离。
  gdformat/gdlint 336 文件无问题，i18n catalog、UI scaling、`git diff --check` 绿色。
- 全量前只临时恢复 B7-0 记录的三张受保护 fixture，三个 SHA-256 完全一致；测试后立即
  删除，无 `.import`、无 raster 暂存。批准计划 SHA-256 仍为
  `655597660e21acdf7a4d5e2bab388bdf54586875ee59921211cdc1dad2f073f4`。
- 到此只建立工程通过。未执行 B7-9、候选构建、真实付费 API、Computer Use、push、
  发布或项目所有者人工签收。
