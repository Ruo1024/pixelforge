...
## 2026-06-19 M3 UX-4 批次语义 LOD 最小骨架

### 本轮实现说明

- 新增 `pixel/ui/canvas/canvas_lod_profile.gd`，集中定义 `overview / review / inspect` 三档语义 LOD 阈值与 inspect 像素网格绘制条件。
- 批次卡接入 LOD：
  - overview：缩小时绘制批次摘要底板、状态分布条和可见/总数，不绘制完整缩略图网格。
  - review：保持现有 contact sheet / focus view 缩略图审阅路径。
  - inspect：在足够放大且缩略图像素尺寸可读时，叠加透明棋盘底与像素网格。
- LOD 使用父级画布 art layer 的缩放判断，避免把外层 shell/UI 缩放误算进画布语义缩放。
- 本轮不提前接管输入仲裁：overview 下仍保留几何缩略图命中，避免破坏已有 mock 批次快捷审阅；“overview 是否禁用缩略图点击”留给 UX-7 Hit-test 与输入仲裁卡统一处理。
- 补 `pixel/CHANGELOG.md` 一行 M3 UX-4 摘要。
- 新增 `pixel/scripts/verify_m3_ux4.sh`，作为本卡出口脚本。

### 验证结果

- `./pixel/scripts/lint.sh`：通过，`106 files would be left unchanged`，`Success: no problems found`。
- `./pixel/scripts/run_tests.sh`：通过，`149/149 tests`，`1229 asserts`。仍有既有 `1 Orphans` 与 Godot 退出资源 warning。
- `./pixel/scripts/verify_m3_ux4.sh`：通过。首次在受限沙箱内被 Godot editor settings 写入拦截；按流程提升权限重跑后通过，包含 lint、全量测试、`check_ui_scaling`、导出模板检查和 staged 图片检查。

### 人工测试步骤

1. 启动 PixelForge，执行 `File > Generate Mock Batch`。
2. 用缩放控件或滚轮逐步缩小画布，确认 Mock Batch 从完整缩略图审阅切到 overview 摘要：能看到批次轮廓、状态分布条和数量。
3. 在默认审阅缩放下，确认缩略图仍可点击选择，`K/R/F` 标记和左右切换仍正常。
4. 放大到 inspect 档，确认缩略图背景出现棋盘格，足够小的像素图上能看到像素网格。
5. 右键切换 `Focus View`，重复缩放检查：review/inspect 下焦点大图和底部 filmstrip 仍显示；overview 下回到摘要。
6. 记录实际手感：overview 下是否应该禁用缩略图点击、阈值是否太早/太晚，作为 UX-7 输入仲裁与后续 LOD 调参依据。

### 本轮完整 diff

```diff
diff --git a/pixel/CHANGELOG.md b/pixel/CHANGELOG.md
index bce660e..a7ffbfb 100644
--- a/pixel/CHANGELOG.md
+++ b/pixel/CHANGELOG.md
@@ -29,3 +29,4 @@
 - M3 G-4 follow-up: graph 连线改用命名端口锚点，修正轻节点端口点、batch 输入点与连线端点错位。
 - M3 G-4 follow-up: AI Generate 画布卡将多个逻辑输入折叠为单个视觉输入点，降低基础节点链噪声。
 - M3 G-5: 新增 File > Run Selected Graph 最小重跑入口，选中 mock 节点链任一节点后可替换刷新正式 batch 队列。
+- M3 UX-4: 新增批次卡语义 LOD 骨架，缩小时显示 overview 状态摘要，正常审阅保留缩略图，放大 inspect 时显示像素网格。
...
+const CanvasLODProfile := preload("res://ui/canvas/canvas_lod_profile.gd")
 ...
 const GraphEdgeRenderer := preload("res://ui/canvas/canvas_graph_edge_renderer.gd")
 const AiGenerateNodeScript := preload("res://core/graph/nodes/ai_generate_node.gd")
@@ -176,6 +177,48 @@ func test_canvas_batch_card_switches_review_layout_for_focus_view() -> void:
 	assert_eq(item["review_layout"], CanvasBatchCardScript.LAYOUT_FOCUS)
...
+const LODProfile := preload("res://ui/canvas/canvas_lod_profile.gd")
 ...
 const CARD_WIDTH := 600
@@ -18,6 +19,11 @@ const BACKGROUND := Color(0.16, 0.17, 0.18, 0.96)
 const BORDER := Color(0.52, 0.62, 0.72, 1.0)
 const SELECTED_BORDER := Color(0.1, 0.85, 0.65, 1.0)
 const THUMB_BACKGROUND := Color(0.08, 0.085, 0.09, 1.0)
...
+const INSPECT_GRID := Color(1.0, 1.0, 1.0, 0.22)
 ...
 const PORT_OUT := Color(0.24, 0.85, 0.58, 1.0)
 const REVIEW_NONE := ""
@@ -41,6 +47,7 @@ const OUTPUT_PORTS: Array[String] = ["images", "assets"]
 const FOCUS_IMAGE_HEIGHT := 320
 const FOCUS_FILMSTRIP_THUMB_SIZE := 72
 const FOCUS_FILMSTRIP_VISIBLE := 7
...
+var _lod_profile_override := ""
 ...
 func setup_from_data(data: Dictionary) -> void:
@@ -236,6 +244,19 @@ func get_review_layout() -> String:
 	return review_layout
...
+		_draw_thumbnail_texture(_texture_asset_id_for(asset_id), rect, lod_profile)
 	...
 	draw_rect(rect, border_color, false, 1.5)
 	_draw_review_marker(rect, String(review_states.get(asset_id, REVIEW_NONE)))
@@ -390,25 +465,25 @@ func _draw_thumbnail(asset_id: String, rect: Rect2) -> void:
 		draw_rect(rect.grow(3.0), FOCUS_BORDER, false, 2.5)
...
+func _draw_thumbnail_texture(asset_id: String, rect: Rect2, lod_profile: String) -> void:
 	...
 	if texture != null:
 		var image_size := texture.get_size()
@@ -416,6 +491,41 @@ func _draw_thumbnail_texture(asset_id: String, rect: Rect2) -> void:
 		var draw_size := image_size * scale
 		var draw_pos := rect.position + (rect.size - draw_size) * 0.5
 		draw_texture_rect(texture, Rect2(draw_pos, draw_size), false)
...
+		)
 ...
 func _draw_review_marker(rect: Rect2, review_state: String) -> void:
@@ -680,6 +790,24 @@ func _filmstrip_start_index(visible_ids: Array[String]) -> int:
 	return clampi(anchor_index - half_window, 0, visible_ids.size() - FOCUS_FILMSTRIP_VISIBLE)
